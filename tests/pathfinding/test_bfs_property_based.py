        ...
        for edges, start, goal, expected_hops in test_cases:
            ...
            if path is not None:
                ...
                assert actual_hops == expected_hops, (
                    f"Expected {expected_hops} hops, got {actual_hops}"
                )
    ...
    def test_bfs_vs_manual_shortest_path(self) -> None:
        ...
        for graph in test_graphs:
            ...
            for _ in range(5):
                ...
                if bfs_path is None:
                    assert nx_path is None, (
                        f"BFS found no path but NetworkX found {nx_path}"
                    )
                else:
                    assert nx_path is not None, (
                        f"BFS found path {bfs_path} but NetworkX found none"
                    )
                    assert len(bfs_path) == len(nx_path), (
                        f"BFS path length {len(bfs_path)} != NetworkX path length {len(nx_path)}"
                    )
    ...
    def test_large_graph_properties(self) -> None:
        ...
        for start, goal in test_cases:
            ...
            if path is not None:
                ...
                assert actual_hops == manhattan_dist, (
                    f"Expected {manhattan_dist} hops, got {actual_hops}"
                )
