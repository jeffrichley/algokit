...
class SnakeQueue(m.VGroup):
    ...
    def _create_panel(self) -> None:
        ...
        # Panel title
        self.panel_title = m.Text(self.title, font_size=18, color=m.WHITE).next_to(
            self.panel_bg, m.UP, buff=0.2
        )
    ...
    def _calculate_positions(self) -> None:
        ...
        bottom_bound = (
            panel_center[1] - panel_height / 2 + padding + self.token_size / 2
        )
    ...
    def enqueue(
        self, token: m.Mobject = None, color: str = m.BLUE_C, scene: m.Scene = None
    ) -> m.Mobject:
        ...
        if scene:
            ...
            scene.play(
                token.animate.move_to([target_pos[0], target_pos[1], 0])
                .scale(scale_factor)
                .set_ease(m.rate_functions.ease_in_out_cubic),
                run_time=0.5,
            )
    ...
    def dequeue(self, scene: m.Scene = None) -> m.Mobject:
        ...
        if scene:
            ...
            scene.play(
                token.animate.move_to(slide_back_pos).set_ease(
