...
class LegendPanel(m.VGroup):
    ...
    def _build_legend(self, items: list[dict[str, Any]]) -> None:
        ...
        for item in items:
            ...
            # Create text label
            label = m.Text(item["label"], font_size=self.font_size, color=m.WHITE)
    ...
    def _create_visual_swatch(self, item: dict[str, Any]) -> m.Mobject:
        ...
        if visual_type == "custom" and "custom_visual" in item:
            ...
            return m.Dot(
                radius=props.get("radius", 0.08), color=props.get("color", m.WHITE)
            )
            ...
            # Default to square
            return m.Square(
                side_length=0.15, fill_color=m.WHITE, fill_opacity=0.6, stroke_width=1
            )
...
def create_custom_legend(items: list[tuple[str, m.Mobject]]) -> LegendPanel:
    ...
    for label, visual in items:
        legend_items.append(
            {"label": label, "visual_type": "custom", "custom_visual": visual}
        )
