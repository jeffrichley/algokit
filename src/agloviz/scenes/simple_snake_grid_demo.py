...
from agloviz.components.snake_queue import SnakeQueue  # type: ignore[import-untyped]
...
class SimpleSnakeQueueDemo(m.Scene):
    ...
    def construct(self) -> None:
        ...
        colors = [m.GREEN_C, m.BLUE_C, m.RED, m.YELLOW, m.PURPLE, m.ORANGE, m.PINK, m.TEAL, 
                 m.GOLD, m.MAROON, m.BLUE, m.LIGHT_GRAY, m.GREEN, m.BLUE_A, m.RED_A, m.YELLOW_A,
                 m.PURPLE_A, m.ORANGE, m.PINK, m.TEAL_A, m.GOLD, m.MAROON_A, m.BLUE_B, m.GRAY]
        tokens = []
        ...
        for i in range(6):
            ...
            self.play(
                token.animate.move_to(target_box.get_center()).set_ease(m.rate_functions.ease_in_out_cubic),
                run_time=0.5
            )
            self.wait(0.2)
        ...
        subtitle = m.Text("Queue Operations: FIFO (First In, First Out)", font_size=20, color=m.GRAY)
