from manim import BLUE, YELLOW, Mobject, Square, VGroup
...
class GridVisualizer:
    ...
    def fill_cell(
        self, x: int, y: int, color: str = BLUE, opacity: float = 1.0
    ) -> None:
        """Permanently fill a cell with color"""
        self.grid_overlay.fill_cell(x, y, color, opacity)

    def highlight_cell(
        self, x: int, y: int, color: str = YELLOW, duration: float = 0.2
    ) -> None:
    ...
    def get_cell_center(self, grid_pos: tuple[int, int]) -> object:
        """Get the center position of a cell."""
        return self.grid_overlay.grid_to_screen(grid_pos)

    def set_cell_fill(
        self, grid_pos: tuple[int, int], color: str, opacity: float
    ) -> Square:
        ...
        """
        return self.grid_overlay.fill_cell(
            grid_pos[0], grid_pos[1], color=color, opacity=opacity
        )

    def set_cell_stroke(
        self, grid_pos: tuple[int, int], color: str, width: float
    ) -> None:
    ...
    def animate_grid_growth(self, scene: object) -> None:
