...
## Goal
Add a short, self-contained section to `README.md` that tells a new contributor exactly how to install the docs dependency group, serve the mkdocs site locally with live reload, and produce a static build. Closes the gap noted in [issue #21](https://github.com/jeffrichley/algokit/issues/21): the existing Quick Start covers package install but never explains how to preview docs changes, even though the project ships an mkdocs-material site under `docs/` and configures it via `mkdocs.yml`.
...
## Approach
This is a single-file README patch — no design pattern applies; it is straightforward documentation. The scope check is the work: stay inside `README.md`, add one section, change nothing else.
...
The single non-obvious decision is the install command. The issue body says the docs deps live in `[project.optional-dependencies]`, which would imply `uv sync --extra docs`. That is wrong: `pyproject.toml` puts `docs = [...]` under `[dependency-groups]` (lines 69-129), and the `justfile`'s `install-docs` recipe canonically uses `uv sync --group docs` (`justfile:143-144`). The acceptance criterion "Commands are accurate to algokit's actual layout — verified against `pyproject.toml` + `mkdocs.yml`" governs here: we follow the layout the repo actually has, not the framing in the issue body.
...
1. **Compose the new section content** as a single contiguous markdown block. Required elements, in this order:
   ...
   - One sentence of lead-in prose explaining what the section covers.
   - A fenced ` ```bash ` block with three commands and inline `#`-comments:
     ```
